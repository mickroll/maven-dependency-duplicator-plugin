...
Now one may argue, that not only moduleX may use the base components, but also its dependent projects. Thinking transicitve dependencies. And those may introduce violating usages of base components, too. Thats right. Use `addDownstream=true` to also execute the tests there and have those covered as well.

There is no need to explicitly use the `test-jar`, you may use another classifier for this. After all, that's just an example use case.
